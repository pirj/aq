...
| `AQ_CPU=<model>` | QEMU `-cpu` model. Default `host` is right for local dev (single-host save/restore). Override to a portable model (e.g. `Skylake-Server-v4`) when cached snapshots will be restored on a different host CPU family — the canonical case is GH `ubuntu-latest` runners, which mix Intel and AMD SKUs. `-cpu host` on AMD bakes `EFER.SVME=1` into the vmstate, which fails VM-entry when restored on Intel (KVM `entry failed, hardware error 0x80000021`). Bump the cache-key-prefix when migrating to a new value so stale snapshots are invalidated. `setup-snapcompose` v3.0.7+ sets this automatically on Linux runners. | `host` |
...
- `AQ_ZST_STREAM_INCOMING=1` — keep the legacy `-incoming exec:pzstd -dc <.zst>` path instead of pre-decompressing memory.bin.zst to disk before `-incoming file:`. Escape hatch for macOS HVF local dev where the streaming path is reliable; the default (v2.5.41+) is the pre-decompress path because Azure x86_64 KVM under Hyper-V hangs on the streaming variant.
- `AQ_CONT_MAX_ATTEMPTS` — override the qmp `cont` retry budget after `-incoming` migration (default 150 × 0.2 s = 30 s). Raise on slow nested-virt CI if you see "VM did not transition to running after N cont attempts".
