...
- [x] **zstd-compress live-snapshot `memory.bin`** — shipped 2026-05-19 (commit b28f881). After `migrate file:memory.bin` + qmp wait, run `zstd -T0 --rm memory.bin -o memory.bin.zst` when zstd is on the host. `aq new --from-snapshot` and `aq start` detect the `.zst` form and feed QEMU via `-incoming exec:zstd -dc <path>` so the decompressed stream lands in QEMU's migration consumer without a temp file. The rails-pg-sample 4 GiB-RAM capture went from 1,638 MiB to 472 MiB on disk (3.47×); warm restore on rlock dropped from 3 s to 2 s (within noise; the smaller I/O cancels the decompression CPU cost on M3 since zstd decompression and QEMU's migration apply run in parallel). Backward-compatible: snapshots from earlier aq versions (plain memory.bin) still restore; hosts without `zstd` get the old uncompressed path with no penalty. See `benchmark-2026-05-19-d-zstd.md` (in the umbrella repo).
