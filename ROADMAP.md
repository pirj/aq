...
- [x] **`AQ_NO_SNAPSHOT_COMPRESS=1` env var to opt out of zstd** — shipped 2026-05-21. For CI runners or single-project laptops where the ~370 ms warm penalty matters more than the ~1.1 GB disk saving. When set, aq writes plain memory.bin and aq_start uses the older `-incoming file:` fast path. Restore path stays compression-aware regardless (older zstd-compressed caches still load fine).
- [ ] **Memory snapshot dedup via `zstd --patch-from` (opt-in)** — when live layers stack (live → live → ...), each layer's compressed `memory.bin.zst` is ~1.2 GB even when only ~200 MB of pages actually changed. zstd 1.4.0+ supports `--patch-from=<reference>` which writes a delta against a decompressed reference; a typical layer-3-on-layer-2 delta of 200 MB raw pages compresses to ~50–80 MB on disk (~95% saving vs standalone). Restoration becomes a host-side chain decompress: distill layer N by sequentially `zstd -d --patch-from=<prev_raw> <this.zstpatch>` from the oldest live ancestor; each step is ~2.5 s on a 4 GB raw (zstd decompresses at ~3-4 GB/s + ~1 s patch apply). qemu's `-incoming file:` consumer stays unchanged — the chain reconstruction is purely a preprocessing step before `aq start`, exactly like the existing `memory.bin.zst → memory.bin` decompress. Gate behind `AQ_MEMORY_PATCH_MODE=1` (and a matching `[snapshot] memory_compression = "patch"` field in bakerish.toml for project-level pinning) so adopters with shallow live chains don't pay the cascade-decompress cost. Sell with: "If your CI cache / OCI cache push is fat with live snapshots, turn this on — trades ~2.5 s per live layer at restore for ~75% less disk per subsequent live layer." Considered (and rejected) alternatives: (a) QEMU's native `xbzrle` migration capability — designed for in-flight host-to-host migration with primed source/dest caches, not for snapshot-to-file chain restore; bending it that way needs non-stable QMP gymnastics and the XOR+RLE compression is significantly weaker than zstd-LZ77 for our delta sizes. (b) Self-written binary diff (bsdiff / xdelta3) — strictly more dependencies and roughly the same patch size as `--patch-from`, no upside.
